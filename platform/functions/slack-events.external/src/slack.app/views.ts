...
export const handleViews = (app: App, _logger: SlackLogger, _publish: PublishFunction<HypsibiusEvent>): void => {
  const EditAlertCallbackID = /label\.(?<labelId>.+?)\.edit\.alert\.config\.(?<index>-?[0-9]+)/gimy;
  ...
  app.view(EditAlertCallbackID, async ({ ack, context, view, client }) => {
    try {
      const { labelId, index } = EditAlertCallbackID.exec(view.callback_id)?.groups ?? {};
      const { startOn, summarizeAbove, onceInType, onceInValue } = Object.fromEntries(
            ...
            alertConfig: [
              {
                ...
                onceInType: onceInType.selected_option?.value
                  ? Object.values(OnceIn).find((v) => v === onceInType.selected_option?.value)
                  : undefined,
                onceInValue: onceInValue.value ? parseInt(onceInValue.value) : undefined,
                summarizeAbove: summarizeAbove.value ? parseInt(summarizeAbove.value) : undefined,
                startOn: startOn.selected_date_time ? new Date(startOn.selected_date_time * 1000) : undefined
              }
            ]
          }
        ]
  ...
  app.view(DeleteAlertsCallbackID, async ({ ack, context, view, client }) => {
    try {
      const { labelId } = DeleteAlertsCallbackID.exec(view.callback_id)?.groups ?? {};
      _logger.error(`Deleting Alerts for label: ${labelId}`);
      const { selected } = Object.fromEntries(Object.values(view.state.values).flatMap((o) => Object.entries(o)));
      if (!context.userId || !context.teamId) {
        throw Error(`No UserId or TeamId defined`);
      }
      const deleteAlerts = selected.selected_options?.map<number>(({ value }) => parseInt(value));
      try {
        const user = await updateLabels({
          userId: context.userId,
          teamOrgId: context.teamId,
  ...
  app.view(EditLabelCallbackID, async ({ ack, context, view, client }) => {
    try {
      const { labelId } = EditLabelCallbackID.exec(view.callback_id)?.groups ?? {};
  ...
  app.view(DeleteLabelsCallbackID, async ({ ack, context, view, client }) => {
    try {
      ...
      await updateLabels({
        userId: context.userId,
        teamOrgId: context.teamId,
        deleteLabelsById: selected.selected_options?.map<string>(({ value }) => value)
      });
      const homeView = await buildHomeView(context.teamId, context.userId);
      const resp = await client.views.publish({
        view: homeView,
